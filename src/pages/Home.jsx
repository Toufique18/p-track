...
//                 />
//                 <button className="btn">filter the products</button>
//             </div>
...
const Home = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:5000/products?page=${currentPage}&limit=10&search=${searchTerm}&brands=${selectedBrands.join(',')}&categories=${selectedCategories.join(',')}&priceRange=${priceRange}`);
                const result = await response.json();
        ...
        fetchData();
    }, [currentPage, searchTerm, selectedBrands, selectedCategories, priceRange]);
